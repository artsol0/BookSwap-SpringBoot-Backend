...
public class AuthenticationService {
    ...
    public String forgotPassword(Map<String, String> request) {
        ...
        if (user != null) {
            String jvtToken = jwtService.generateToken(user);
            saveUserToken(user, jvtToken);
            String link = "http://localhost:8080/api/v1/forgot-password/reset-password?token=" + jvtToken;
            emailSender.send(user.getEmail(), "Reset password", buildResetPasswordText(user.getNickname(), link));
    ...
    private String buildConfirmText(String name, String link) {
    ...
    private String buildResetPasswordText(String name, String link) {
        return "<div style=\"font-family:Helvetica,Arial,sans-serif;font-size:16px;margin:0;color:#0b0c0c\">\n" +
                "\n" +
                "<span style=\"display:none;font-size:1px;color:#fff;max-height:0\"></span>\n" +
                "\n" +
